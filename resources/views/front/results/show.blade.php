        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <table class="mt-4 table w-full table-view">
                        ...
                        @if(auth()->user()?->is_admin)
                            <tr class="w-28">
                                <th class="border border-solid bg-slate-100 px-6 py-3 text-left text-sm font-semibold uppercase text-slate-600">
                                    User
                                </th>
                        ...
                        <tr class="w-28">
                            ...
                            <td class="border border-solid px-6 py-3">
                                ...
                                @if($test->time_spent)
                                    (time: {{ (int) ($test->time_spent / 60) }}:{{ gmdate('s', $test->time_spent) }}
                                    minutes)
                                @endif
                            </td>
                        </tr>
                ...
                @if($users)
                    ...
                    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 pb-12 my-2">
                            ...
                            <div class="p-6 text-gray-900">
                                ...
                                <table class="table mt-4 w-full table-view">
                                    ...
                                    @forelse($users as $user)
                                        <tr @class(['bg-slate-100' => auth()->user() && auth()->user()->name == $user->name, 'text-left'])>
                                            <td class="w-9 p-2">{{ $loop->iteration }}</td>
                                            <td class="w-1/2 p-2">{{ $user->name }}</td>
                                            <td class=" p-2">{{ $user->correct }} / {{ $total_questions }}
